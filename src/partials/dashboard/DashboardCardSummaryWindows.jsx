...
import logoWindows from '../../images/windows-logo-01.svg';
...
function DashboardCardSummaryWindows({ collectorHost }) {
  ...
  const getNodeData=()=>{
    ...
    .then((res)=>res.json())
    .then((topologydata)=>{
      const nodes = topologydata.nodes.filter(function(windowsnode) {
        return windowsnode.os.toUpperCase() == "WINDOWS";
      });
      console.log(nodes)
      setNodeData(nodes)
    })
        ...
        {nodeData.map((node) => (
          ...
          <li key={node.id} className="col-span-1 divide-y rounded-lg shadow-lg divide-stone-300 bg-stone-50 shadow-slate-300">
            <div className="flex items-center justify-between w-full p-6 space-x-6">
              ...
              <img className="flex-shrink-0 w-10 h-10 bg-gray-100 rounded-full shadow-inner shadow-gray-700" src={logoWindows} alt="" />
            </div>
            <div>
...
export default DashboardCardSummaryWindows;
